...
function MyApp({ Component, pageProps }) {
  ...
  if(!isMetamaskConnected){
    return (
      <>
        <Nav />
        <div className="flex items-center justify-center w-full h-screen">
          <h1 className="text-3xl font-bold">No Metamask Connection Detected , Use The Connect To Metamask Button</h1>
        </div>
      </>
    )
  }
  ...
  if(!isOnCorrectChain){
    return (
      <>
        <Nav />
        <div className="flex items-center justify-center w-full h-screen">
          <h1 className="text-3xl font-bold">Wrong Network , Connect To { process.env.NEXT_PUBLIC_ENV == 'prod' ? 'Polygon Mumbai' : 'Ganache' } :(</h1>
        </div>
      </>
    )
  }
  ...
  return(
    <>
      <Head>
        <title>Diverse - Uniswap v2</title>
